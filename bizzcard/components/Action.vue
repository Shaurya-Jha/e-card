<template>
    <div class="flex mt-6">
        ...
        <div class="p-3 shrink-0 rounded-1" :style="{ background: `${name == 'secondaryActions' ? item.color : buttonBg}` }"
            :title="item.name">
            <div class="w-6 h-6" :class="name == 'secondaryActions' ? null : 'action'" v-html="getSvg(item)"></div>
        </div>
    </div>
    ...
    <div class="w-full">
        <input type="text" ref="input"
            ...
            :aria-label="`Enter ` + item.label" :title="`Enter ` + item.label" v-model="type[index].value"
        ...
        <button
            ...
            @click="removeAction(name, index)" :aria-label="'Remove ' + item.label" title="Remove field">
