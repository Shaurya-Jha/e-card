<template>
    <div class="flex items-center mt-2">
        ...
        <div class="flex flex-col items-center bg-gray-700 rounded p-2">
            <div class="flex items-center w-full">
                <div id="imageContainer" class="mr-2 shrink-0">
                    <img :src="item.image.dataURI" :alt="item.image.title"
                        ...
                        v-if="item.image && item.image.dataURI" title="Click to remove product image"
                        @click="removeImage(i)" @keypress.space.enter.prevent="removeImage(i)" tabindex="0">

                    <button v-else
                        class="p-3 h-12 w-12 box-border rounded cursor-pointer border border-dashed border-black hover:border-gray-400 transition-colors duration-200 focus:outline-none"
                        @click="loadFile(i)" aria-label="Add product image" :class="dragOver ? 'outline-white' : ''"
                        @drop.prevent="fileLoaded($event, i, true)" @dragleave.prevent.self="dragOver = false"
                        @dragover.prevent.self="dragOver = true">
                        <input type="file" ref="import" accept="image/jpeg, image/png" v-show="false"
                            @change="fileLoaded($event, i, false)" @click="$event.target.files = null">

                        <div class="w-6 h-6 pointer-events-none" v-html="require(`~/assets/icons/add-img.svg?include`)">
                        </div>
                    </button>
                </div>
                ...
                <div class="w-full">
                    <input type="text"
                        class="px-4 w-full h-12 bg-block placeholder-gray-600 rounded border border-transparent transition-colors duration-200 focus:outline-none focus:border-gray-500 hover:border-gray-500"
                        ref="input" v-model="item.title" autocapitalize="words" aria-label="Enter product title"
                        title="Enter product title" placeholder="Product title">
                </div>
            </div>

            <!-- product description -->
            <textarea name="description" rows="2" placeholder="Product description"
                ...
                v-model="item.description" aria-label="Enter product description"
                title="Enter product description"></textarea>
            ...
            <input type="text" name="price"
                ...
                autocapitalize="words" placeholder="Price" v-model="item.price" aria-label="Enter product price"
            ...
            <div class="grid grid-cols-2 gap-x-2">
                ...
                <input type="text" name="link"
                    ...
                    placeholder="Button Link" v-model="item.link" aria-label="Enter button link" title="Enter button link">
                ...
                <input type="text" name="label"
                    ...
                    placeholder="Button label" autocapitalize="words" v-model="item.label" aria-label="Enter button label"
        ...
        <button
            ...
            @click="removeItem(i)" aria-label="Remove product" title="Remove product">
    ...
    methods: {
        ...
        imageLoaded(file, i, mime) {
            ...
            reader.onload = (f) => {
                ...
                this.resizeImage('product', mime, this.index, this.featured[this.index].content.length - 1)
            }
